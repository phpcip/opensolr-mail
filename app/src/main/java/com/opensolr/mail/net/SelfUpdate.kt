...
object SelfUpdate {
    ...
    private const val ACTION_RESULT = "com.opensolr.mail.SELF_UPDATE_RESULT"

    /** True when Google Play installed this copy: then Play does the updating, not the app. */
    fun fromPlay(context: Context): Boolean {
        if (com.opensolr.mail.BuildConfig.PLAY_BUILD) return true
        val installer = try {
    ...
    fun canInstall(context: Context): Boolean = !com.opensolr.mail.BuildConfig.PLAY_BUILD && context.packageManager.canRequestPackageInstalls()
    ...
    private fun download(context: Context, url: String, onProgress: (Int) -> Unit): File {
        require(url.startsWith("https://github.com/phpcip/opensolr-mail/")) { "unexpected update address" }
        val dir = File(context.cacheDir, "update").apply { mkdirs() }
        dir.listFiles()?.forEach { it.delete() }
        val out = File(dir, "opensolr-mail.apk")
    ...
    private fun install(context: Context, file: File) {
        ...
        installer.openSession(id).use { session ->
            file.inputStream().use { input ->
                session.openWrite("opensolr-mail.apk", 0, file.length()).use { output ->
                    input.copyTo(output)
