    ...
    suspend fun search(text: String, filters: Filters, groupBy: GroupBy, start: Int = 0, rows: Int = 40, byRelevance: Boolean = false): Result {
        ...
        fun parse(docs: JSONArray): List<Hit> = (0 until docs.length()).map { i ->
            ...
            if (h != null) hlMap[id] = mapOf(
                ...
                // Only the message's own words go to the AI answer, never an attachment's.
                "text" to (h.optJSONArray("body_t")?.let { a -> (0 until a.length()).map { a.getString(it) } } ?: emptyList()),
            )
            Hit(
                ...
                folders = d.optJSONArray("mailbox_name_ss")?.let { a -> (0 until a.length()).map { a.optString(it) }.filter { it.isNotBlank() } }.orEmpty(),
